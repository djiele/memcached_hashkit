...
/* {{{ void memcached_hashkit_md5_signature( [ string $var ] )
 */
PHP_FUNCTION(memcached_hashkit_md5_signature) 
{
	...
	if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa", &var, &var_len, &results) == FAILURE) {
		return;
	}
    ...
    md5_signature((unsigned char*)var, (unsigned int)var_len, results);
    
    RETURN_NULL();
}
/* }}}*/

/* {{{ string memcached_hashkit_md5( [ string $var ] )
 */
PHP_FUNCTION(memcached_hashkit_md5) 
{
    ...
    RETURN_LONG(((uint32_t) (results[3] & 0xFF) << 24)
